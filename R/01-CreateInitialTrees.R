...
in_dir <- "01_data"
out_dir_sl <- file.path("02_init_sortie","02_summit_lake","ParameterValues")
out_dir_dc <- file.path("02_init_sortie","01_date_creek","ParameterValues")

#1. You need access to raw data to generate the Sortie init files:
raw_dat_sl <- "D:/Github/SummitLakeData/data-raw/SummitLakeData.csv"
raw_tree_dc <- "D:/Github/DateCreekData/data-raw/Trees/1992data.csv"
raw_tally_dc <- "D:/Github/DateCreekData/data-raw/Trees/1992fixed_radius_data_fromTable20_DateCkHandbook.csv"

#2. You need access to the spatial boundaries to implement the harvest in Date Creek:
block_bounds <- ReadSpatialBounds("D:/Github/DateCreekData/data-raw/Harvests_Plants/UnitBoundaries/")

#3. You need access to planting data to implement post-harvest plan in low retention (gap)
# and no retention (clearcut) treatments
#planting data
PlantDensDat <- fread("D:/Github/DateCreekData/data-raw/Harvests_Plants/PlantingDensities.csv", 
                      na.strings="n/a", stringsAsFactors = TRUE)
PlantPCDat <- fread("D:/Github/DateCreekData/data-raw/Harvests_Plants/PlantingPercents.csv",
                    na.strings="n/a", 
                    stringsAsFactors = TRUE)
plant_behav_pos <- "Plant21" #need to change to reflect where the planting behaviour is within the Sortie param file
#SBS---------------------------------------------------------------------------
...
Live_dt <- createTreeListSortie(cruise_data = raw_tree_dc,
                                fixed_data = tally_dat,
                                liveTrees = TRUE)
Live_dt[!grepl("^Init\\.Dens_", DiamClass),DiamClass := paste0("Init.Dens_",DiamClass)]
Dead_dt <- createTreeListSortie(cruise_data = raw_tree_dc,
                                fixed_data = tally_dat,
                                liveTrees = FALSE)
...
# read spatial boundaries and integrate harvest and planting into Sortie file
...
pl_dens <- PlantDensDat[,.(DiamClass ="PlantingDens", 
                           Amabalis_Fir=sum(.SD)),by=c("Unit","GM")]
...
for(j in 1:length(Blocks)){
  ...
  if(TreatType=="CC"||TreatType=="HR"||TreatType=="LR"){ 
    ...
    if(TreatType=="HR"){
      ...
      e <- rbind(d,data.table(DiamClass=plant_behav_pos,Amabalis_Fir=NA),fill=TRUE)
  ...
  write.csv(jj,file.path(out_dir_dc,paste0(Unit_i,".csv")), quote=TRUE, row.names=FALSE)
}
