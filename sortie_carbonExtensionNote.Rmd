...
ls <- append(ls, c("DateCreekData","SummitLakeData","sortieCarbon")) #data wrangling packages 
#ls <- append(ls, c("DateCreekData","SummitLake_Data")) #data wrangling packages - add summit lake when created
ls <- append(ls, c("treeCalcs")) #package for allometry and carbon calculations
...
1. Calculate carbon from SORTIE
```{r}
#TO DO - ADD SEEDLINGS TO CARBON CALCS
######### Date Creek -----------------------------------------------------------------------
path_outputs <- "D:/Github/SORTIEparams/Outputs/ICH/Snags" #update this to the latest run path
...
######## Summit Lake ------------------------------------------------------------------------
path_outputs <- "D:/Sync/BVRC/FSD program/FCI/Partial cutting/Manuscript/Extension Note/SORTIE Carbon Extension Note/Files/Outputs/Extracted"
run_name <- "-0506e"
...
#merge with treatment data from SummitLakeData package:
sl_sortie <- merge(sl_sortie, SummitLakeData::Treatments, by.x = "Unit", by.y = "unit")

fwrite(sl_sortie,"./Inputs/Data/SL_sortieC.csv")
...
```{r}
NH_tr <- DT_LS[Treatment=="NH"]
NH_tr[, UniqXY := paste0(X,"_",Y), by = seq_len(nrow(NH_tr))]
setkey(NH_tr,UniqXY)

#1. Label the row with whether it's a sapling, snag created, or snag fall
ids <- unique(NH_tr$UniqXY)
for(i in 1:length(ids)){
  #looking for adults, and snags that will fall
  NH_tr[UniqXY == ids[i], stType := ifelse(Type == "Adult","Adult",
                                      ifelse(Type == "Snag" & Dead.Code =="Alive",
                                             "Snag",
                                        ifelse(Type=="Snag" & Dead.Code == "Natural",
                                              "SnFallNext", "Sapling")))]
  # identify if/when snag created
  snagCh <- which(NH_tr[UniqXY == ids[i]]$Type != 
              dplyr::lag(NH_tr[UniqXY == ids[i]]$Type))
  if(length(snagCh) > 0){
    NH_tr[UniqXY == ids[i]][snagCh]$stType <- "SnagCreate"  
  }
}

#2. Calculate Snag creation and snag fall rate
NH_ad <- NH_tr[stType=="Adult", .N, by=.(Treatment, Unit, timestep)]
setnames(NH_ad, "N", "NumAdult")
NH_sn <- NH_tr[stType=="SnagCreate", .N, by=.(Treatment, Unit, timestep)]
setnames(NH_sn, "N", "NumSnags")
NH_sf <- NH_tr[stType=="SnFallNext", .N, by=.(Treatment, Unit, timestep)]
setnames(NH_sf, "N", "NumSnagFall")

NH_ad_sn <- merge(NH_ad, NH_sn, by=c("Treatment","Unit","timestep"))
NH_ad_sn <- merge(NH_ad_sn, NH_sf, by=c("Treatment","Unit","timestep"))
NH_ad_sn <- NH_ad_sn[,.(NumAdult, NumSnags, NumSnagFall,
                SnagRecrRate = (NumSnags/(NumAdult+NumSnags))*100, 
                SnagFallRate = (NumSnagFall/(NumSnags+NumSnagFall))*100),
             by=c("Treatment","Unit","timestep")]

ggplot(NH_ad_sn)+
  geom_point(aes(x = timestep, y = SnagRecrRate, colour = Unit),size=4)+
  geom_line(aes(x = timestep, y = SnagRecrRate, colour = Unit),size=1.5)+
  theme_minimal()+
  ylab("Snag Recruitment Rate (%)")
#ggsave("D:/Github/sortie_carbonExtensionNote/Outputs/Figures/SnagRecruitRate.jpg")

ggplot(NH_ad_sn)+
  geom_point(aes(x = timestep, y = SnagFallRate, colour = Unit),size=4)+
  geom_line(aes(x = timestep, y = SnagFallRate, colour = Unit),size=1.5)+
  theme_minimal()+
  ylab("Snag Fall Rate (%)")
#ggsave("D:/Github/sortie_carbonExtensionNote/Outputs/Figures/SnagFallRate.jpg")

#3. how long are snags standing
#of the snags that fall, how long were they standing? So first figure out which trees fall
fallIDs <- unique(NH_tr[stType == "SnFallNext"]$UniqXY)

#then count how many years they stood as snags
for(i in 1:length(fallIDs)){
  NH_tr[UniqXY == fallIDs[i] & Type == "Snag", TimeAsSnag := .N]
}

SnagTime <- NH_tr[UniqXY %in% fallIDs & stType == "SnFallNext", .(Treatment,Unit,Species,DBH,UniqXY,TimeAsSnag)]
...
FS_cwd_sl <- F_cwd_sl[,.(MgHa = mean(MgHa), VolHa = mean(MgHa)),
                      by = c("treatment","unit","timestep","DecayClass")]

MS_cwd_sl <- cwdc_sm[,.(MgHa = mean(MgHa), VolHa = mean(values)), 
                     by = c("treatment","unit","timestep")]
FS_cwd_sl <- F_cwd_sl[,.(MgHa = mean(MgHa), VolHa = mean(VolHa)),
                      by = c("treatment","unit","timestep")]
FS_cwd_sl_s <- F_cwd_sl_s[,.(MgHa = mean(MgHa), VolHa = mean(VolHa)),
                      by = c("treatment","unit","timestep")]
...
ggplot(NULL)+
  geom_line(data = MS_cwd_dc, aes(x= Yrs_Post, y = VolHa, 
                                  group = Unit, colour = Unit), linetype = 1)+
  geom_line(data = FS_cwd_dc, aes(x=Yrs_Post, y = VolHa, 
                                  group = Unit, colour = Unit), linetype = 2)
...
# Sum by year, block, treatment
plotTots <- F_cwd_dc[,sum(VolumeHa), by = .(Unit, Year, Unique_plot)]
plotTots[,mean(V1), by = .(Unit, Year)]

#need tog et all the zeros for sp-decay combos and merge with species groups for each unique plot:
...
#add the units and years:

#plotUnit <- data.table(Unit = rep(DateCreekData::Treatments$Unit,4),
 #                      Year = c(rep(1992,16),rep(1993,16),rep(2011,16),rep(2019,16)))
...
plotSp <- expand.grid(unique(F_cwd_dc$Unique_plot), unique(F_cwd_dc$Sp))

merge(spGrTabl)



#for now looping - but change to lapply? - over each plot
for(ii in 1:length(unique(F_cwd_dc$Unique_plot))){
  merge(F_cwd_dc, spGrTabl, by = c("Sp","Decay"), all.y = TRUE)
}


F_cwd_dc_0s <- merge(F_cwd_dc, spGrTabl, by = c("Sp","Decay"), all.y = TRUE)




plotTots <- F_cwd_dc[,.(plotVol = sum(VolumeHa)), by = .(Unit, Year, Sp, Decay, Unique_plot)]
plotMn <- plotTots[,.(plotmn = mean(plotVol)), by = .(Unit, Sp, Decay, Year)]



F_cwd_dc[, .(VolumeHa= mean(VolumeHa)), by=c("Block","Unit", "Treatment", "Year", "PostHarvest")]


FS_cwd_dc <- F_cwd_dc[,.(MgHa = mean(MgHa), VolHa = mean(VolumeHa)),
                      by = c("Treatment","Unit","Yrs_Post")]
MS_cwd_dc[,volha_cal := VolHa*10, by=seq_len(nrow(MS_cwd_dc))]

calibr <- merge(FS_cwd_dc[Yrs_Post < 19], MS_cwd_dc,
                by= c("Treatment","Unit","Yrs_Post"))
calibr[, VolHa.x/VolHa.y, by = seq_len(nrow(calibr))]
